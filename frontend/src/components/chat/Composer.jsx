...
export default function Composer({ onSend, onStop, busy, disabled, placeholder, leading }) {
  ...
  return (
    ...
    >
      ...
      />
      {busy ? (
        <Button type="button" size="icon" variant="secondary" onClick={onStop} aria-label={t('composer.stop')} className="shrink-0 rounded-xl">
          <Square className="size-4 fill-current" />
        </Button>
      ) : (
        <Button type="submit" size="icon" disabled={disabled || !value.trim()} aria-label={t('composer.send')} className="shrink-0 rounded-xl">
